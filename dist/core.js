...
export function sessionStorageLike() {
    try {
...
export function readReloadedFor(storage, storageKey) {
    try {
...
export function writeReloadedFor(storage, storageKey, version) {
    try {
        if (!storage)
            return;
        if (version === null)
 ...
 */
export async function checkServerVersion(serverVersionPath, fetchImpl = globalThis.fetch) {
    try {
